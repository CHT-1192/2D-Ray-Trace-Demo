...
`src/config.ts` 里的 `THEME` 基本来自对参考截图的采样（灰度 0..1）：方块填充 `0x3D3D3D`、远处背景 `0.315`、
辉光振幅 `0.3`、定向光占比 `0.62`、灯泡剖面 `lightStops`、棱边高光强度 `rimStrength`。

其中**光照范围是唯一刻意偏离参考图的参数**：参考图拟合出来是半径 `700` / 指数 `1.16`，只够铺到画面中部
（最远的角上辉光只剩 1.4%）；现在放大到半径 `1050` / 指数 `1.0`，亮光铺满整个画面，
最远的角上仍有 35%，阴影在更远处也还看得清。想改回去就调这两个值。

`src/core/layout.ts` 里是尺寸分布（`SIZE_CLASSES` / `ASPECT_FIT` / `HEIGHT_RANGE`）与放置带推导；`src/core/scene.ts` 里是生成参数（`SPAWN_MARGIN` / `CULL_MARGIN` / `MIN_GAP` / 横向步进）与可复现性实现；默认种子在 `config.ts` 的 `DEFAULT_SEED`。

`DEFAULT_OPTIONS` 里是默认模式、初速度（70 参考单位/秒）、uniform 射线数、ε 等。
