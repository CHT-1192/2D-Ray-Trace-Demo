...
- 像素级配色：方块填充 `0x3D3D3D`、远处背景 `0.318`、亮区衰减曲线误差 ≤ 0.3/255、灯泡径向剖面误差 ≤ 10/255；
- 硬阴影：半径 150 的圆环上同时存在亮区与阴影区，亮度差 > 25；
...
`src/config.ts` 里的 `THEME` 全部来自对参考截图的采样（灰度 0..1）：方块填充 `0x3D3D3D`、远处背景 `0.315`、辉光振幅 `0.3` / 半径 `700` / 指数 `1.16`、定向光占比 `0.62`、灯泡剖面 `lightStops`、棱边高光强度 `rimStrength`。
