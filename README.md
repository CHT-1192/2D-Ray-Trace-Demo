...
- `npm run verify` —— Playwright + Chromium 端到端验证（53 项，见下文）
...
npm run verify      # 53 项：Playwright + Chromium 端到端
```
...
- **高级面板**：按 `A` 能开合、26 个滑块在线；拖「光照范围」角落立刻变亮、拖「阴影深度」整图均值下降、
  「恢复默认值」后画面与改动前**逐像素一致**；
...
- **高级面板**（`A` 键，或主面板底部的「高级参数」按钮）：**26 个实时参数**，分六组
...
| 组 | 参数 |
| --- | --- |
| 光照 | 光照范围、衰减指数、辉光亮度、环境光、阴影深度、棱边高光 |
